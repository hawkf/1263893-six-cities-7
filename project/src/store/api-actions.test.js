...
describe('Async operations', () => {
  ...
  it('should make a correct API call to GET /offers/:offerId', () => {
    const apiMock = new MockAdapter(api);
    const dispatch = jest.fn();
    const id = 2;
    ...
    return offerLoader(dispatch, () => {}, api)
      .then(() => {
        ...
        expect(dispatch).toHaveBeenNthCalledWith(1, {
          ...
          payload: updateOfferToClient(offers[1]),
        });
      })
      .catch(() => {
        expect(dispatch).toHaveBeenCalledTimes(1);
        expect(dispatch).toHaveBeenNthCalledWith(1, {
          ...
          payload: AppRoute.PAGE_NOT_FOUND,
        })
      })
  });
  ...
  it('should make a correct API call to POST /comments/:offerId', () => {
    const apiMock = new MockAdapter(api);
    const dispatch = jest.fn();
    const id = 2;
    const commentPost = postComment({comment: 'test', rating: 4}, id, () => {});
